...
The site is plain static HTML/CSS/JS in [`public/`](public/) with no server-side build step. At load, [`public/js/main.js`](public/js/main.js) parses [`public/sites.csv`](public/sites.csv) with [PapaParse](https://www.papaparse.com/) (vendored at [`public/js/papaparse.min.js`](public/js/papaparse.min.js) so there is no third-party request) and renders one card per row.
